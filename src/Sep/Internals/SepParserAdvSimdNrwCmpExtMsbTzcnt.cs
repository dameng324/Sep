...
sealed class SepParserAdvSimdNrwCmpExtMsbTzcnt : ISepParser
{
    ...
    public unsafe SepParserAdvSimdNrwCmpExtMsbTzcnt(SepParserOptions options)
    {
    ...
    LOOPNOSTEP:
            ...
            !IsAddressLessThan(ref colInfosRefStop, ref colInfosRefCurrent))
        {
            ...
            // Narrow both 16-bit vectors to 8-bit with saturation
            var narrow0 = AdvSimd.ExtractNarrowingSaturateLower(v0);
            var narrow1 = AdvSimd.ExtractNarrowingSaturateLower(v1);
            var bytes = Vector128.Create(narrow0, narrow1); // AdvSimd.Combine(narrow0, narrow1);
