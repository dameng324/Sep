...
sealed class SepParserVector128NrwCmpExtMsbTzcntUnroll2X : ISepParser
{
    ...
    LOOPNOSTEP:
            ...
            !IsAddressLessThan(ref colInfosRefStop, ref colInfosRefCurrent))
        {
            ...
            var qtsEq0 = Vec.Equals(bytes0, qts);
            var spsEq0 = Vec.Equals(bytes0, sps);

            var nlsEq1 = Vec.Equals(bytes1, nls);
            var crsEq1 = Vec.Equals(bytes1, crs);
            var qtsEq1 = Vec.Equals(bytes1, qts);
            var spsEq1 = Vec.Equals(bytes1, sps);

            var lineEndings0 = nlsEq0 | crsEq0;
            var lineEndingsSeparators0 = spsEq0 | lineEndings0;
            var specialChars0 = lineEndingsSeparators0 | qtsEq0;

            var lineEndings1 = nlsEq1 | crsEq1;
            var lineEndingsSeparators1 = spsEq1 | lineEndings1;
            var specialChars1 = lineEndingsSeparators1 | qtsEq1;
